  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' export MAJOR_VERSION=$(curl -sL https://download.foldingathome.org/js/fah-downloads.js | grep 'series = typeof _series == ' | awk -F\' '{print $4}') && curl -sL https://download.foldingathome.org/releases.py?series=${MAJOR_VERSION} | awk -F'(fahclient_|_amd64.deb)' '/debian-stable-64bit/ {print $2;exit}' ''',
            returnStdout: true).trim()
